...
export const SystemLogs: React.FC<SystemLogsProps> = ({ isLargeText }) => {
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `System_Logs_BanCo_${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  };
              ...
              {loading ? (
                ...
                paginatedLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-8 py-5">
                      <p className={`font-black uppercase tracking-tight ${isLargeText ? 'text-base' : 'text-sm'} ${log.action.includes('Khóa') || log.action.includes('Sửa') ? 'text-admin-red' :
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`size-9 rounded-lg flex items-center justify-center text-xs font-black transition-all ${currentPage === pageNum
